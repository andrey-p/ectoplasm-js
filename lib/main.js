...
exports.initialise = exports.initialize = function (scripts, callback) {
  /*jslint unparam: true*/
  httpServer = http.createServer(function (req, res) {
    ...
    socketServer.sockets.on("connection", function (socket) {
      ...
      socketConnection.on("output", function (id, args) {
        // output event means no error, so pad out with empty first error argument
        args.unshift(null);
        getCallbackWithId(id).apply(null, args);
      });
      socketConnection.on("err", function (id, message) {
        getCallbackWithId(id)(message);
      });
